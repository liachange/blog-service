...
// Aliyun 实现sms.Driver interface
type Aliyun struct{}

// Send 实现sms.Driver interface的Send方法
func (s *Aliyun) Send(phone string, message Message, config map[string]string) bool {
	...
	logger.DebugJSON("", "接口响应", result)
	if err != nil {
		logger.ErrorString("短信[阿里云]", "短信发送失败", err.Error())
		return false
	}
	...
	if result.IsSuccessful() {
		logger.DebugJSON("短信[阿里云]", "短信发送成功", "")
		return true
	} else {
