...
var (
	...
	ErrTokenInvalid           error = errors.New("令牌请求无效")
...
func (jwt *JWT) RefreshToken(c *gin.Context) (string, error) {
	...
	clamis := token.Claims.(*JWTCustomClaims)
	// 5. 检查是否过了『最大允许刷新的时间』
	x := app.TimenowInTimezone().Add(-jwt.MaxRefresh).Unix()
	if clamis.IssuedAt > x {
		// 修改过期时间
		clamis.StandardClaims.ExpiresAt = jwt.expireAtTime()
		return jwt.createToken(*clamis)
	}
